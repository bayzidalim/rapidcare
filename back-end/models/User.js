...
class User {
  ...
  /**
   * Get hospital authorities for a specific hospital
   * @param {number} hospitalId - Hospital ID
   * @returns {Array} - Array of hospital authority users
   */
  static getHospitalAuthorities(hospitalId) {
    const stmt = db.prepare(`
      SELECT u.* 
      FROM users u
  ...
  /**
   * Get all hospital authorities
   * @returns {Array} - Array of all hospital authority users
   */
  static getAllHospitalAuthorities() {
    const stmt = db.prepare(`
  ...
  /**
   * Associate user with hospital (for hospital authorities)
   * @param {number} userId - User ID
   * @param {number} hospitalId - Hospital ID
   * @returns {object} Updated user
   */
  static associateWithHospital(userId, hospitalId) {
    const stmt = db.prepare(`
      UPDATE users 
      SET hospital_id = ?, updatedAt = CURRENT_TIMESTAMP
      WHERE id = ? AND userType = 'hospital-authority'
    `);
    
    const result = stmt.run(hospitalId, userId);
    
    if (result.changes === 0) {
      throw new Error('User not found or not a hospital authority');
    }
  ...
  /**
   * Remove hospital association from user
   * @param {number} userId - User ID
   * @returns {object} Updated user
   */
  static removeHospitalAssociation(userId) {
    const stmt = db.prepare(`
      UPDATE users 
      SET hospital_id = NULL, updatedAt = CURRENT_TIMESTAMP
      WHERE id = ?
    `);
   ...
   */
  static hasHospitalAccess(userId, hospitalId) {
    ...
    // Hospital authorities only have access to their assigned hospital
    if (user.userType === 'hospital-authority') {
      return user.hospital_id === hospitalId;
    }
    
    // Regular users can view hospitals but not manage them
    return false;
  }
   ...
   * @param {number} hospitalId - Hospital ID
   * @returns {Array} Array of users associated with the hospital
   */
  static getByHospitalId(hospitalId) {
    const stmt = db.prepare(`
      SELECT * FROM users 
      WHERE hospital_id = ? AND isActive = 1
