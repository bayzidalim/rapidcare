...
const PaymentWorkflow: React.FC<PaymentWorkflowProps> = ({
  ...
  const calculateBookingAmount = async () => {
    try {
      ...
      if (result.success) {
        setTotalAmount(result.data.finalAmount || result.data.calculatedAmount);
      } else {
        // Fallback to booking payment amount if calculation fails
        setTotalAmount(bookingData.payment.amount);
      }
    } catch (error) {
